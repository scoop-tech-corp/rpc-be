...
class StaffController extends Controller
{
    ...
    public function SearchDataStatic(Request $request)
    {
        ...
        $dataJobTitle = DB::table('jobTitle as a')
            ->select(
                'a.id',
                DB::raw("'Job Title' as type"),
                'a.jobName as typeName',
            )
            ->where('a.isActive', '=', 1);


        $dataTypeId = DB::table('typeId as a')
            ->select(
                'a.id',
                DB::raw("'Type of Id' as type"),
                'a.typeName as typeName',
            )
            ->where('a.isActive', '=', 1);


        $dataPayPeroid = DB::table('payPeriod as a')
            ->select(
                'a.id',
                DB::raw("'Pay Period' as type"),
                'a.periodName as typeName',
            )
            ->where('a.isActive', '=', 1);


        $dataStaticUsage = DB::table('payPeriod as a')
            ->select(
                'a.id',
                DB::raw("'Pay Period' as type"),
                'a.periodName as typeName',
        ...
        $dataStaticUsage = DB::table('data_static as a')
            ->select(
                'a.id',
        ...
        $dataStaticTelephone = DB::table('data_static as a')
            ->select(
                'a.id',
        ...
        $dataStaticMessenger = DB::table('data_static as a')
            ->select(
                'a.id',
