...
class DataProductInventoryHistory implements FromCollection, ShouldAutoSize, WithHeadings, WithTitle, WithMapping
{
    use Exportable;
    ...
    public function __construct($orderValue, $orderColumn, $fromDate, $toDate, $search, $locationId, $userRole)
    {
    ...
    public function collection()
    {
        ...
        if ($this->fromDate && $this->toDate) {
            $data = $data->whereBetween(DB::raw('DATE(p.created_at)'), [$this->fromDate, $this->toDate]);
        }

        if ($this->search) {
            $res = $this->SearchHistory($this->search);
            if ($res) {
    ...
    private function SearchHistory($search)
    {
        $temp_column = null;
    ...
    {
        return 'Riwayat Produk Inventory';
    }
